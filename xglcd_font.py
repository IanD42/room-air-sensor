    ...
    def __load_xglcd_font(self, path):
        ...
        with open(path, 'r') as f:
            for line in f:
                ...
                while line.endswith(','):
                    line = line[:-1].strip()
                # Parse hex values, skip empty tokens
                values = []
                for b in line.split(','):
                    b = b.strip()
                    if b and b.startswith('0x'):
                        try:
                            values.append(int(b, 16))
                        except ValueError:
                            pass
                # Only store if correct length
                if len(values) != bytes_per_letter:
                    continue
                for i, v in enumerate(values):
                    self.letters[offset + i] = v
